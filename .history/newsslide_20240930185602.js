document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchNews() {
        try {
            ...
            if (response.ok) {
                ...
                // Устанавливаем currentIndex так, чтобы отображались последние три новости
                currentIndex = newsList.length >= 3 ? newsList.length - 3 : 0;
    ...
    function updateSlider() {
        ...
        // Добавляем 3 карточки, начиная с currentIndex
        for (let i = 0; i < 3; i++) {
            visibleNews.push(newsList[(currentIndex + i) % newsList.length]);
        }
        ...
        visibleNews.forEach((news, index) => {
            ...
            if (index === 2) {
                newsCard.classList.add('right-most'); // Обозначаем самую актуальную новость справа
            }
