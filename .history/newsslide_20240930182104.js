    ...
    async function fetchNews() {
        try {
            ...
            if (response.ok) {
                ...
                // Проверка формата даты для каждой новости
                newsList.forEach(news => {
                    console.log(`Дата до преобразования: ${news.datePublished}`);
                });
                ...
                newsList.sort((a, b) => {
                    ...
                    console.log(`Дата после преобразования A: ${dateA}, B: ${dateB}`); // Диагностика дат
    
                    return dateB.getTime() - dateA.getTime();
                });
    
                // Устанавливаем currentIndex так, чтобы самая актуальная новость оказалась в центре
                if (newsList.length > 1) {
                    currentIndex = 1; // Поскольку мы отображаем 3 карточки, ставим самую актуальную в центр
                } else {
                    currentIndex = 0; // Если новость всего одна, она будет первой
                }
